...
STATES = [
    ...
    # "WY",  # RIDB hangs indefinitely on WY — still gets manual state parks via fetch_wy_state_parks
]


# ── GEOCODE CACHE ──────────────────────────────────────────────────────
# Persistent key→[lat, lon] store in data/geocode_cache.json.
# Keys are query strings (Nominatim) or "il:<slug>" (IL coordinate scraping).
...
def load_geocode_cache() -> None:
    """Load the geocode cache from disk into _geocode_cache. Safe to call if file is absent."""
    ...
    if GEOCODE_CACHE_FILE.exists():
        try:
            ...
            if isinstance(data, dict):
                _geocode_cache = data
                print(f"  Geocode cache loaded: {len(_geocode_cache)} entries from {GEOCODE_CACHE_FILE.relative_to(REPO_ROOT)}")
            else:
                print(f"  WARNING: {GEOCODE_CACHE_FILE.name} is not a JSON object — starting with empty cache")
        except Exception as e:
            print(f"  WARNING: Could not load geocode cache ({e}) — starting with empty cache")
    else:
        print(f"  Geocode cache not found — will create {GEOCODE_CACHE_FILE.relative_to(REPO_ROOT)} on first run")


def write_geocode_cache() -> None:
    """Write _geocode_cache back to disk, sorted by key for stable diffs."""
    if _geocode_stats["misses"] == 0:
        return  # Nothing new to write
    try:
        sorted_cache = dict(sorted(_geocode_cache.items()))
        GEOCODE_CACHE_FILE.write_text(
            json.dumps(sorted_cache, indent=2, ensure_ascii=False) + "\n",
            encoding="utf-8",
        )
        print(f"  Geocode cache saved: {len(_geocode_cache)} entries → {GEOCODE_CACHE_FILE.relative_to(REPO_ROOT)}")
    except Exception as e:
        print(f"  WARNING: Could not write geocode cache: {e}")


def _cache_get(key: str) -> tuple[float, float] | None:
    """Return cached (lat, lon) for key, or None on a miss."""
...
def _cache_set(key: str, lat: float, lon: float) -> None:
    """Store a result in the cache (only if coordinates are valid)."""
...
def strip_html(text):
    ...
    return _strip_html_basic(text or "")
...
_FETCH_FAILED = object()  # sentinel: network/HTTP error, not an empty result
...
def normalize_description_text(value):
    """Keep JSON description values as one readable line. Delegates to _clean_text."""
    ...
    for param_key, param_val in search_terms:
        ...
        while True:
            ...
            if data is _FETCH_FAILED:
                print(f"  RIDB fetch failed for {state} ({param_key}={param_val}) at offset {offset} — skipping remaining pages")
                break
            if not data:
                break
...
def fetch_nps_state(state):
    ...
    if data is _FETCH_FAILED or not data:
        if data is _FETCH_FAILED:
            print(f"  NPS fetch failed for {state} — skipping")
        return camps
...
def fetch_ca_state_parks():
    ...
    while True:
        ...
        if data is _FETCH_FAILED:
            print(f"  CA State Parks fetch failed at offset {offset} — skipping remaining pages")
            break
        if not data:
            break
...
def fetch_il_state_parks():
    ...
    for site_name, main_url in deduped_sites:
        # Derive slug early so we can check the coordinate cache before any HTTP.
        slug = ""
        ...
        cache_key = f"il:{slug}" if slug else f"il-url:{main_url}"
        ...
        if cached_coords is not None:
            ...
            if (lat is None or lng is None) and slug:
                ...
                if fallback_main:
                    ...
                    phone, lat, lng, _ = _il_extract_phone_coords(fallback_main)
...
def _clean_text(value):
    """Collapse whitespace and strip. Alias for normalize_description_text."""
...
def fetch_fl_state_parks():
    ...
    for idx, name in enumerate(park_names, start=1):
        lat, lon = geocode_nominatim(f"{name}, Florida")
        if abs(lat) < 0.1 or abs(lon) < 0.1:
            continue
...
def fetch_mi_state_parks():
    ...
    for name in mi_sites:
        lat, lng = geocode_nominatim(f"{name}, Michigan")
        if abs(lat) < 0.1 or abs(lng) < 0.1:
            lat, lng = geocode_nominatim(f"{name} campground, Michigan")
        if abs(lat) < 0.1 or abs(lng) < 0.1:
            lat, lng = geocode_nominatim(f"{name} equestrian campground, Michigan")
        if abs(lat) < 0.1 or abs(lng) < 0.1:
            continue
...
def merge_state(camps, all_camps):
    """Merge a list of state-park camps into all_camps by ID, returning the count added."""
    new_count = 0
    for camp in camps:
        cid = camp["id"]
...
def main():
    ...
    for abbr, state_name, fetcher, source_label in state_park_jobs:
        ...
        merged = merge_state(state_camps, all_camps)
    ...
    print("\nGeocode cache:")
    print_metric("Cache hits", _geocode_stats["hits"])
    print_metric("Cache misses (new)", _geocode_stats["misses"])
    write_geocode_cache()
