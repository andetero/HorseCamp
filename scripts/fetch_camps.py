...
# ── KANSAS STATE PARKS ─────────────────────────────────────────────────
# Kansas is discovered dynamically from the current official KDWP State Parks
# directory first. Every current park page is inspected for overnight horse/equestrian
# camping evidence. Older official indexes/publications remain guarded fallbacks.
...
KS_OVERNIGHT_HORSE_CAMP_PATTERNS = [
    re.compile(r"\bequestrian\s+(?:camp(?:ground|ing)?|camps?|sites?|area)\b", re.I),
    re.compile(r"\bhorse\s+(?:camp(?:ground|ing)?|camps?|sites?|area)\b", re.I),
    re.compile(
        r"\b(?:campground|camping|campsites?|camp\s+sites?)\b.{0,140}\b(?:horse|equestrian)\b",
        re.I | re.S,
    ),
    re.compile(
        r"\b(?:horse|equestrian)\b.{0,140}\b(?:campground|camping|campsites?|camp\s+sites?)\b",
        re.I | re.S,
    ),
    re.compile(r"\bcampsites?\s+with\s+corrals?\b", re.I),
]

KS_HORSE_CAMP_NEGATIVE_PATTERNS = [
    re.compile(r"\bno\s+(?:horse|equestrian)\s+camp(?:ing|ground)?\b", re.I),
    re.compile(r"\bhorses?\s+(?:are\s+)?not\s+allowed\s+in\s+(?:the\s+)?campground\b", re.I),
    re.compile(r"\bday\s+use\s+only\b", re.I),
    re.compile(r"\bno\s+overnight\s+camp(?:ing)?\b", re.I),
]
...
def _ks_directory_page_urls(raw_html, page_url):
    """Discover pagination URLs from the official KDWP State Parks directory."""
    base_path = urlparse(KS_STATE_PARKS_BASE).path.rstrip("/")
    urls = []
    seen = set()
    anchor_re = re.compile(
        r"<a\b[^>]*href=[\"']([^\"'#]+(?:\?[^\"'#]*)?)[\"'][^>]*>([\s\S]*?)</a>",
    ...
    for match in anchor_re.finditer(raw_html or ""):
        href = html.unescape(match.group(1)).strip()
        label = re.sub(r"\s+", " ", _strip_html_basic(match.group(2))).strip().lower()
        absolute = urljoin(page_url or KS_STATE_PARKS_BASE, href)
        parsed = urlparse(absolute)
        host = parsed.netloc.lower().split(":", 1)[0]
        path = re.sub(r"/{2,}", "/", parsed.path or "/").rstrip("/")
        if host not in {"ksoutdoors.gov", "www.ksoutdoors.gov", "ksoutdoors.com", "www.ksoutdoors.com"}:
            continue
        if path != base_path or not parsed.query:
            continue

        # Current KDWP pagination is generated by the site platform. Accept the
        # usual page/start/offset forms and explicit numbered/next navigation.
        if not (
            label in {"next", "next »", "last »", "last", "first", "« first", "prev", "previous", "« prev"}
            or label.isdigit()
            or re.search(r"(?:^|[?&])(?:page|start|offset|currentpage|pagenum|pageindex)=", parsed.query, re.I)
        ):
            continue

        normalized = f"https://www.ksoutdoors.gov{path}?{parsed.query}"
        if normalized not in seen:
            seen.add(normalized)
...
def _ks_directory_park_links(raw_html, page_url):
    """Discover current park root pages from the official KDWP State Parks directory."""
    base_path = urlparse(KS_STATE_PARKS_BASE).path.rstrip("/")
    rows = []
    seen = set()
    anchor_re = re.compile(
    ...
    for match in anchor_re.finditer(raw_html or ""):
        label = re.sub(r"\s+", " ", _strip_html_basic(match.group(2))).strip()
        url = _ks_official_url(match.group(1), page_url or KS_STATE_PARKS_BASE)
        if not url:
            continue
        path = urlparse(url).path.rstrip("/")
        if not re.fullmatch(re.escape(base_path) + r"/[^/]+", path):
            continue
        if path in seen:
            continue
        seen.add(path)
...
def _ks_same_park_candidate_links(park_url, raw_html):
    """Find current KDWP subpages likely to contain horse-camping details."""
    base_path = urlparse(park_url).path.rstrip("/")
    links = []
    seen = {park_url.rstrip("/")}
    anchor_re = re.compile(
        r"<a\b[^>]*href=[\"']([^\"'#]+)(?:[?#][^\"']*)?[\"'][^>]*>([\s\S]*?)</a>",
    ...
    for match in anchor_re.finditer(raw_html or ""):
        ...
        if not parsed.path.startswith(base_path + "/"):
            continue
        blob = f"{parsed.path} {label}".lower()
        if not any(term in blob for term in ("horse", "equestrian", "equine", "camp", "area")):
            continue
        normalized = f"https://www.ksoutdoors.gov{parsed.path.rstrip('/')}"
        if normalized in seen:
            continue
        seen.add(normalized)
        links.append(normalized)
...
def _ks_page_confirms_overnight_horse_camping(raw_html):
    """Require current official evidence of overnight horse/equestrian camping."""
    text = _usfs_html_to_text(raw_html)
    if any(pattern.search(text) for pattern in KS_HORSE_CAMP_NEGATIVE_PATTERNS):
        return False
    if any(pattern.search(text) for pattern in KS_OVERNIGHT_HORSE_CAMP_PATTERNS):
        return True

    horse_contexts = _ks_horse_contexts(raw_html)
    return bool(horse_contexts and _ks_select_horse_area(raw_html, horse_contexts))


def _ks_discover_from_state_parks_directory():
    """Inspect every current park in KDWP's canonical State Parks directory."""
    final_url, first_html = _ks_fetch_html(KS_STATE_PARKS_BASE)
    if not first_html:
        print("  Kansas State Parks: canonical State Parks directory unavailable")
        return []

    first_url = final_url or KS_STATE_PARKS_BASE
    directory_pages = [(first_url, first_html)]
    queued = set(_ks_directory_page_urls(first_html, first_url))
    visited = {first_url}

    # Follow pagination dynamically, with a hard cap against malformed loops.
    while queued and len(directory_pages) < 10:
        page_url = sorted(queued)[0]
        queued.remove(page_url)
        if page_url in visited:
            continue
        visited.add(page_url)

        final_page_url, page_html = _ks_fetch_html(page_url, referrer=KS_STATE_PARKS_BASE)
        if not page_html:
            continue
        resolved = final_page_url or page_url
        directory_pages.append((resolved, page_html))
        for discovered in _ks_directory_page_urls(page_html, resolved):
    ...
    for page_url, page_html in directory_pages:
        for park_url, label in _ks_directory_park_links(page_html, page_url):
            if park_url in seen_urls:
                continue
            seen_urls.add(park_url)
            park_rows.append((park_url, label))

    if not park_rows:
        print("  Kansas State Parks: canonical directory contained no park links")
        return []

    print(
        f"  Kansas State Parks: discovered {len(park_rows)} current park pages "
        f"from {len(directory_pages)} canonical directory page(s)"
    )

    rows = []
    for park_url, directory_label in park_rows:
        final_park_url, park_html = _ks_fetch_html(park_url, referrer=KS_STATE_PARKS_BASE)
        if not park_html:
            continue

        # Start with the park root. If it does not itself prove overnight horse
        # camping, inspect a bounded set of current same-park official subpages.
        evidence_html = park_html
        confirmed = _ks_page_confirms_overnight_horse_camping(park_html)

        if not confirmed:
            for candidate_url in _ks_same_park_candidate_links(final_park_url or park_url, park_html):
                _, candidate_html = _ks_fetch_html(candidate_url, referrer=final_park_url or park_url)
                if not candidate_html:
                    continue
                if _ks_page_confirms_overnight_horse_camping(candidate_html):
                    evidence_html = park_html + "\n" + candidate_html
                    confirmed = True
                    break
                time.sleep(0.15)
        ...
        park_name = _al_extract_h1(park_html) or directory_label
        if not park_name:
            time.sleep(random.uniform(*KS_STATE_PARKS_DELAY_SECONDS))
            continue
        ...
        rows.append({
            "name": park_name,
            "park_url": final_park_url or park_url,
            "source_url": KS_STATE_PARKS_BASE,
            "phone": "",
            "campsites": 0,
            "trail_miles": 0.0,
            "source_kind": "state-parks-directory",
            "park_html": evidence_html,
        })
        time.sleep(random.uniform(*KS_STATE_PARKS_DELAY_SECONDS))

    print(
        f"  Kansas State Parks: canonical directory confirmed "
        f"{len(rows)} current horse-camping parks"
    )
    return rows
...
def _ks_discover_official_rows():
    # Preferred source: the current official KDWP State Parks directory.
    rows = _ks_discover_from_state_parks_directory()
    if rows:
        return rows

    print(
        "  Kansas State Parks: canonical State Parks directory produced no "
        "horse-camp rows; trying alternate official sources"
    )

    # Older official equestrian HTML lists remain fallbacks only.
    for source_url in KS_STATE_PARKS_HTML_SOURCES:
        ...
        if rows:
            print(
                f"  Kansas State Parks: discovered {len(rows)} "
                "equestrian-campground parks from fallback official HTML"
            )
            return rows
        print(
            f"  Kansas State Parks: fallback official HTML source had no "
            f"campground links: {source_url}"
        )

    # Official KDWP publication fallback.
    for source_url in KS_STATE_PARKS_PUBLICATION_SOURCES:
        _, pdf_bytes = _ks_request(source_url, binary=True)
        if not pdf_bytes:
            continue
        publication_text = _ks_pdf_to_text(pdf_bytes)
        rows = _ks_publication_rows(publication_text, source_url)
        declared = _ks_declared_publication_count(publication_text)
        ...
        if rows:
            print(
                f"  Kansas State Parks: discovered {len(rows)} horse-camping parks "
                "from official KDWP publication"
            )
            return rows
        print(
            f"  Kansas State Parks: KDWP publication produced no horse-camp rows: "
            f"{source_url}"
        )
...
def fetch_ks_state_parks():
    ...
    for row in rows:
        ...
        final_url = park_url
        park_html = str(row.get("park_html") or "")
        if not park_html and park_url:
