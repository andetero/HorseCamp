...
AK_HORSE_ACCESS_PATTERNS = [
    ...
    re.compile(r"\bhorse\s+trails?\b", re.I),
    re.compile(r"\bhorses?\s+(?:are\s+)?allowed\b", re.I),
...
def _ak_official_park_url(href, base_url=AK_STATE_PARKS_CAMPGROUND_INDEX):
    ...
    parsed = urlparse(absolute)
    if parsed.netloc.lower() != "dnr.alaska.gov":
        return ""
    path = re.sub(r"/{2,}", "/", parsed.path or "/")
    if not path.lower().startswith("/parks/aspunits/"):
        return ""
...
def _ak_parse_positive_int(text):
...
def _ak_page_hookups(main_text, directory_row=None):
    ...
    if re.search(r"\b50\s*[- ]?amp\b|\b50a\b", main_text, flags=re.I):
        hookups.append("50A")
    elif re.search(r"\b30\s*[- ]?amp\b|\b30a\b|\belectric(?:al)?\s+hookups?\b", main_text, flags=re.I):
        hookups.append("30A")

    if re.search(r"\bsewer\s+hookups?\b|\bfull\s+hookups?\b", main_text, flags=re.I):
        hookups.append("Sewer")

    return list(dict.fromkeys(hookups))


def _ak_rig_length(main_text):
    match = re.search(r"\bRV\s+Length\s+Limit\s*[-:]\s*(\d+)\s*(?:ft|feet|foot)\b", main_text, flags=re.I)
    return int(match.group(1)) if match else 0


def _ak_page_phone(main_text):
    match = re.search(
        r"\b(?:Phone|Park\s+Office)\s*:?\s*(?:\+?1[-. ]*)?"
        r"\(?(\d{3})\)?[-. ]+(\d{3})[-. ]+(\d{4})\b",
        main_text,
        flags=re.I,
    )
...
def _ak_build_dynamic_camp(directory_row, final_url, raw_html, regulation):
    ...
    community = str(directory_row.get("community") or "").strip()
    location = f"{community}, AK" if community else f"{page_name}, AK"
    ...
    if abs(lat) < 0.1 or abs(lon) < 0.1:
        print(f"  Alaska State Parks: could not resolve coordinates for {page_name}")
        return None
    ...
    return {
        ...
        "seasonStart": 0,
        "seasonEnd": 0,
        "hasWashRack": bool(re.search(r"\b(?:wash\s+rack|horse\s+wash)\b", main_text, flags=re.I)),
        "hasDumpStation": bool(re.search(r"\b(?:sanitary\s+dump|RV\s+dump\s+station|dump\s+station)\b", main_text, flags=re.I)),
        "hasWifi": bool(re.search(r"\b(?:wi[- ]?fi|internet)\b", main_text, flags=re.I)),
        "hasBathhouse": bool(re.search(r"\b(?:bathhouse|showers?|flush\s+toilets?)\b", main_text, flags=re.I)),
        "pullThroughAvailable": bool(re.search(r"\bpull[- ]through\b", main_text, flags=re.I)),
        "imageColors": ["C0392B", "F1948A"],
        "photoURLs": [],
        "source": "State Parks",
    }
...
def fetch_ak_state_parks():
    ...
    _, index_html = _fetch_state_park_html(
        ...
        "Alaska State Parks campground index",
        user_agent=AK_STATE_PARKS_USER_AGENT,
    )
    if not index_html:
...
# ── KANSAS STATE PARKS ─────────────────────────────────────────────────
# Kansas dynamic sourcing is intentionally paused. Keep the previously curated
# Kansas State Parks data until a reliable official dynamic source is available.
